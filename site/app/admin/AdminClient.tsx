  ...
  return (
    <main className="admin-shell">
      <aside className="admin-sidebar">
        <div className="admin-sidebar-top"><Link className="brand admin-brand" href="/">{settings.brandName}<span>。</span></Link><ThemeToggle /></div>
        <nav><button className={section==="home"?"selected":""} onClick={()=>setSection("home")}>首页设置 <b>01</b></button><button className={section==="articles"?"selected":""} onClick={()=>setSection("articles")}>文章管理 <b>{stats.total}</b></button><button className={section==="page"?"selected":""} onClick={()=>setSection("page")}>关于设置 <b>03</b></button><button className={section==="categories"?"selected":""} onClick={()=>setSection("categories")}>分类管理 <b>{categories.length}</b></button><Link href="/" target="_blank">查看前台 ↗</Link><button onClick={signOut}>安全退出 <b>↗</b></button></nav>
        ...
        <div className="table-wrap">
          <table><thead><tr><th>文章</th><th>分类</th><th>状态</th><th>浏览</th><th>发布时间</th><th>操作</th></tr></thead>
            <tbody>{loading ? <tr><td colSpan={6} className="table-empty">正在读取文章…</td></tr> : posts.length === 0 ? <tr><td colSpan={6} className="table-empty">没有符合条件的文章</td></tr> : posts.map(post => (
              <tr key={post.id}><td><div className="title-cell"><span>{post.id}</span><div><b>{post.title}</b><small>/{post.slug}</small></div></div></td><td>{post.categoryName}</td><td><i className={`status ${post.status}`} />{post.status === "published" ? "已发布" : "草稿"}</td><td>{post.viewCount.toLocaleString()}</td><td>{post.publishedAt ? new Date(post.publishedAt).toLocaleDateString("zh-CN") : "—"}</td><td><div className="row-actions"><button onClick={() => edit(post)}>编辑</button><button className="danger" onClick={() => remove(post)}>删除</button></div></td></tr>
            ))}</tbody></table>
        </div>
